    ...
    const SerializationByName & serializations)
{
    ...
    serialization->enumerateStreams(enumerate_settings,
        [&](const ISerialization::SubstreamPath & substream_path)
        {
            ...
            streams.emplace_back(ISerialization::getFileNameForStream(column, substream_path));
        },
        data);
...
String getColumnNameInStorage(const String & name, const NameSet & storage_columns_name_set)
{
    ...
    {
        String base_name(split.first);
        if (storage_columns_name_set.contains(base_name))
            return base_name;
    }

    throw Exception(ErrorCodes::LOGICAL_ERROR, "Column or subcolumn '{}' is not found in storage columns", name);
}
...
void VerticalInsertTask::classifyColumns()
{
    ...
    {
        const String & stat_column = stat->columnName();
...
void VerticalInsertTask::buildOffsetGroups(const NamesAndTypesList & storage_columns)
{
    ...
    for (const auto & column : storage_columns)
    {
        ...
        array_size_streams.emplace_back(getArraySizeStreams(column, block, serializations));
    }
...
void VerticalInsertTask::addVerticalColumnsSample(const Block & sample)
{
    for (const auto & column : sample)
    {
        if (vertical_columns_sample_names.insert(column.name).second)
            vertical_columns_samples.emplace_back(column);
    }
}
...
void VerticalInsertTask::cancel() noexcept
{
    ...
    vertical_columns_samples.clear();
    vertical_columns_sample_names.clear();
}
...
struct VerticalInsertTask::VerticalPhaseState
{
    ...
    void init()
    {
        ...
        resetBatch();
        task.vertical_columns_samples.clear();
        task.vertical_columns_sample_names.clear();
    ...
    void flushBatch()
    {
        ...
        task.addVerticalColumnsSample(delayed.stream->getColumnsSample());
    ...
    void finish()
    {
        /// Flush remaining batch and finalize delayed streams.
        flushBatch();
