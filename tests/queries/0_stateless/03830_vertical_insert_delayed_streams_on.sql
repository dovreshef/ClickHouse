-- Test: vertical insert with delayed streams enabled (max_insert_delayed_streams_for_parallel_write=2).
SET materialize_skip_indexes_on_insert = 1;

DROP TABLE IF EXISTS t_vi_delayed_streams_on;

CREATE TABLE t_vi_delayed_streams_on
(
    id UInt64,
    c1 UInt64,
    c2 UInt64,
...
SET max_insert_delayed_streams_for_parallel_write = 2;

INSERT INTO t_vi_delayed_streams_on
...
WHERE type = 'QueryFinish'
  ...
  AND Settings['max_insert_delayed_streams_for_parallel_write'] = '2'
  AND query LIKE 'INSERT INTO t_vi_delayed_streams_on%'
...
WHERE database = currentDatabase()
  AND table = 't_vi_delayed_streams_on'
  AND active
  AND secondary_indices_uncompressed_bytes > 0;

DROP TABLE t_vi_delayed_streams_on;
